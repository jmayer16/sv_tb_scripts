...
def writeTestbench(cp, vp, tbfile):
   ...
   tbfile.write("\treset = 1'b1;\n\t#(8*half"+cp.clocks[0][0]+")\n")
   tbfile.write("\treset = 1'b0;\nend\n")
...
def writeDoFile(cp, vp, dofile):
   ...
   for var in vp.inputs:
      if (int(var[1])+1) > 7 or (int(var[1])+1)%4 == 0:
         ...
         value = 'binary'
      dofile.write("add wave -color Green -label {"+var[0]+"} -radix "+value)
      dofile.write(" /"+cp.uut_name+"_tb/uut/"+var[0]+"\n")
   dofile.write("\n")
   for var in vp.internals:
      dofile.write("add wave -color Blue -label {"+var[0]+"} -radix "+var[1])
      dofile.write(" /"+cp.uut_name+"_tb/uut/"+var[0]+"\n")
   dofile.write("\n")
   for var in vp.outputs:
      if (int(var[1])+1) > 7 or (int(var[1])+1)%4 == 0:
         ...
         value = 'binary'
      dofile.write("add wave -color Yellow -label {"+var[0]+"} -radix "+value)
      dofile.write(" /"+cp.uut_name+"_tb/uut/"+var[0]+"\n")
   #write run statement
   dofile.write("\nrun "+cp.runtime)
   return
